#!/bin/bash
# Generate minimal FK grants for external schemas only (Docker version)
# Usage: ./generate_minimal_fk_grants_docker.sh <container> <db_user> <db_pass> <target_user> <prefix1> [prefix2...]
...
if [ -z "$CONTAINER" ] || [ ${#CONTROLLED_PREFIXES[@]} -eq 0 ]; then
    echo "Usage: $0 <container> <db_user> <db_pass> <target_user> <prefix1> [prefix2...]"
    echo ""
    echo "Example:"
    echo "  $0 mysql-fk-test root tutorial user1 'one\\_%' 'two\\_%'"
    echo ""
...
echo "-- Save to file: $0 ... > grants.sql"
echo "-- Apply: docker exec -i $CONTAINER mysql -u$DB_USER -p$DB_PASS < grants.sql"
